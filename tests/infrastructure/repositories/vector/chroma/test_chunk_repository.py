...
from src.infrastructure.repositories.vector.chroma.chunk_repository import ChunkChromaRepository
    ...
    def test_hybrid_search(self, mock_emb, monkeypatch):
        ...
        with patch("chromadb.HttpClient") as mock_http:
            ...
            with patch("langchain_chroma.Chroma") as mock_chroma_class:
                ...
                mock_col = MagicMock()
                mock_client.get_collection.return_value = mock_col
                mock_col.get.return_value = {"ids": [cid], "documents": ["hybrid text"], "metadatas": [mock_doc.metadata]}
