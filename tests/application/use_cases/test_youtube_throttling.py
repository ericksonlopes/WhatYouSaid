...
def test_throttling_logic(monkeypatch):
    ...
    monkeypatch.setattr(use_case, "_process_single_video", lambda *args: {"video_url": "url", "created_chunks": 1})
    monkeypatch.setattr(use_case, "_resolve_subject", lambda *args: SimpleNamespace(id=uuid.uuid4(), name="s"))
    ...
    monkeypatch.setattr(YoutubeExtractor, "extract_playlist_videos", lambda url: ["v1", "v2", "v3", "v4", "v5"])
