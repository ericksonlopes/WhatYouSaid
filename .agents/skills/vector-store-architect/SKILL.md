...
- **Vectors**: Do not store raw vectors inside the `ChunkEntity`. The vector store backend handles embedding indexing natively based on the `content`.

## 🛠️ Workflows

### Implementing a New Vector Store
1. Create the folder `src/infrastructure/repositories/vector/<new_store>/`.
2. Create `<new_store>_repository.py`.
...
- **DO NOT** use raw DB clients directly inside Streamlit frontend pages. Always call a Service that calls the Repository.
