...
class Crawl4AIExtractor(IBaseExtractor):
    ...
    async def extract(self, source: str, **kwargs: Any) -> List[Document]:
        ...
        max_sub_pages = kwargs.get("max_sub_pages", 10)  # Limit to 10 sub-pages per crawl
        ...
        try:
            async with AsyncWebCrawler(config=self.browser_config) as crawler:
                ...
                if not result.success:
                    logger.error(f"Crawl4AI failed to scrape {source}: {result.error_message}")
                ...
                documents.append(Document(page_content=main_markdown, metadata=metadata))

                # 2. Handle Depth 2
                if depth > 1:
                    logger.info(f"Depth {depth} requested. Extracting links from {source}...")
                    
                    # Get internal links
                    internal_links = result.links.get("internal", [])
                    sub_urls = list(set([link["href"] for link in internal_links if link.get("href") and link["href"].startswith("http")]))
                    ...
                    if sub_urls:
                        ...
                        if sub_urls:
                            logger.info(f"Following {len(sub_urls)} internal links for Depth 2", context={"sub_urls": sub_urls})
                            ...
                            for sub_res in sub_results:
                                if sub_res.success:
                                    ...
                                    documents.append(Document(page_content=sub_res.markdown, metadata=sub_metadata))
            ...
            logger.error(f"Unexpected error during Crawl4AI extraction: {e}", context={"url": source})
            raise e
